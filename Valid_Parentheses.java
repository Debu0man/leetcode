...
class Solution {
    ...
    public boolean isValid(String s) {
        Stack<Character> parentheses = new Stack<>(); // Stack 객체 생성
        for (int i = 0; i < s.length(); i++) { // 문자열의 길이만큼 반복
            ...
            if (mappings.containsKey(c)) { // 문자형 변수에 있는 값이 map안에 있는 key에 있을 시
                parentheses.push(mappings.get(c)); // 스택에 문자형 변수 값과 동일한 map key의 value를 스택에 저장
            } else if (mappings.containsValue(c)) { // 문자형 변수에 있는 값이 map안에 value중에 있을 시
                if (parentheses.isEmpty() || parentheses.pop() != c){ // 스택이 비어 있거나 스택을 추출했을 때 값(문자형 변수의 값과 동일한 map key의 value)이 문자형 변수와 다를 시
                    return false; // false 리턴
                }
            }
        }
        return parentheses.isEmpty(); // 객체가 비어 있으면 true
    }
}
